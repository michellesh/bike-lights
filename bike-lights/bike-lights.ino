...
void setup() {
  ...
  FastLED
      .addLeds<WS2813, LED_PIN, GRB>(leds, NUM_LEDS) // WS2815s
  ...
  pattern = PATTERN_ATOM; // PATTERN_SOUND;
...
void loop() {
  ...
  } else if (pattern == PATTERN_ATOM) {
    // fadeToBlackBy(leds, STRAND_LENGTH, 30);
    FastLED.clear();
  } else {
    FastLED.clear();
  }
  ...
  EVERY_N_MILLISECONDS(100) {
    ...
    if (pattern == PATTERN_SOUND) {
      ...
      value = analogRead(COLOR_PIN);
      knobColor = CHSV(map(value, 4095, 0, 0, 255), 255, 255);
    }
...
void barSumBike() {
  ...
  for (int s = 0; s < numSegments; s++) {
    int height = map(sum, 0, NUM_LEDS, 0, segmentLength[s] * 2);
    for (int x = 0; x < height; x++) {
